...
function createBallpit(e, t = {}) {
  ...
  return {
    ...
    updateSizes(minSize, maxSize) {
      if (s) {
        s.config.minSize = minSize;
        s.config.maxSize = maxSize;
        s.physics.config.minSize = minSize;
        s.physics.config.maxSize = maxSize;
        s.physics.setSizes();
      }
    },
    togglePause() {
...
const Ballpit = ({ 
  ...
  useEffect(() => {
    ...
    // Default colors - vibrant gradient for visibility
    ...
    const responsiveMinSize = minSize !== undefined ? minSize : (isMobile ? 0.2 : 0.4);
    const responsiveMaxSize = maxSize !== undefined ? maxSize : (isMobile ? 0.4 : 0.8);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (newIsMobile !== isMobile) {
        ...
        // Just update sizes without recreating
        const responsiveMinSize = minSize !== undefined ? minSize : (newIsMobile ? 0.2 : 0.4);
        const responsiveMaxSize = maxSize !== undefined ? maxSize : (newIsMobile ? 0.4 : 0.8);
        spheresInstanceRef.current.updateSizes(responsiveMinSize, responsiveMaxSize);
      }
    };

    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, [isMobile, minSize, maxSize]);
